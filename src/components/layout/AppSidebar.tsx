...
export function AppSidebar({
  ...
  return <Sidebar className="border-r border-border/30 bg-card/50 backdrop-blur-xl" collapsible="icon">
            ...
            <SidebarMenu>
              {navItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild tooltip={item.title}>
                    <NavLink to={item.url} end={item.url === '/'} className="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 text-muted-foreground hover:text-foreground hover:bg-primary/10 group" activeClassName="bg-primary/20 text-primary border border-primary/30 shadow-[0_0_15px_-3px_hsl(var(--primary)/0.3)]">
